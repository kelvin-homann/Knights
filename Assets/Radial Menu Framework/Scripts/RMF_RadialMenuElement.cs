...
public class RMF_RadialMenuElement : MonoBehaviour {
    ...
    void Start () {
        ...
        defaultPos = button.transform.localPosition;
    ...
    {
        //Update position
        Vector2 pos = defaultPos + new Vector2(0, visible ? (active ? activeOffset : 0) : invisibleOffset);
        button.transform.localPosition = Vector3.Lerp(button.transform.localPosition, pos, Time.deltaTime * 20);

        //Update visiblity
        cg.alpha = Mathf.Lerp(cg.alpha, visible ? 1.0f : 0f, Time.deltaTime * parentRM.fadeSpeed * 2);
    }
    ...
    public void highlightThisElement(PointerEventData p, bool showInfo=true) {
